...
export class RapidAPIClient {
  ...
  async getProfileData(linkedinUrl: string): Promise<any> {
    try {
      ...
      const response = await axios.get(
        `${this.baseURL}/get-profile-data-by-url`,
        {
          headers: this.headers,
          params: {
            url: linkedinUrl,
      ...
      console.error(
        `❌ Error fetching profile data for ${linkedinUrl}:`,
        error.message,
      );
      ...
      if (error.response) {
        ...
        if (status === 429) {
          throw new Error(
            `Rate limit exceeded for ${linkedinUrl}. Please try again later.`,
          );
        } else if (status === 404) {
          throw new Error(`Profile not found: ${linkedinUrl}`);
        } else if (status === 403) {
          throw new Error(
            `Access denied for ${linkedinUrl}. Profile might be private.`,
          );
        } else {
          throw new Error(
            `API error (${status}): ${data?.message || "Unknown error"}`,
  ...
  async getProfilesData(
    ...
    console.log(
      `🚀 Processing ${linkedinUrls.length} LinkedIn URLs in batches of ${batchSize}`,
    );
    ...
    for (let i = 0; i < linkedinUrls.length; i += batchSize) {
      ...
      console.log(
        `📦 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(linkedinUrls.length / batchSize)}`,
      );
    ...
    console.log(
      `✅ Processing complete: ${successful.length} successful, ${failed.length} failed`,
    );
