...
async function testProfileUpdate() {
  ...
  try {
    ...
    const response = await fetch(`${BASE_URL}/api/linkedin/sync`, {
      ...
      body: JSON.stringify({
        linkedinUrl: 'https://www.linkedin.com/in/test-user-profile'
      })
    });
    const duration = Date.now() - start;

    if (response.ok || response.status === 404) {
      console.log(`✅ Profile update test passed (${duration}ms)`);
      if (response.status === 404) {
        console.log(`   Note: 404 is expected (candidate not found)`);
      }
    } else {
      console.log(`❌ Profile update test failed: HTTP ${response.status}`);
    }
  } catch (error: any) {
