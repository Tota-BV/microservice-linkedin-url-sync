...
export function TextViewEdit({
  title,
  name,
}: { title: string; name: "overview" | "referencesAndProjects" }) {
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <EditSection name={name} title={title}>
      ...
      <CardContent>
        <p className="whitespace-pre-line">{profile?.[name] || "..."}</p>
      </CardContent>
    </Card>
  );
}
...
function EditSection({
  ...
  title: string;
  name: "overview" | "referencesAndProjects";
}>) {
  const { profile } = useLoaderData({ from: "/(app)/profile/" });
